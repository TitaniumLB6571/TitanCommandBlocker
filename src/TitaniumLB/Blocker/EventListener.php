...
class EventListener implements \pocketmine\event\Listener
{
    ...
    public function __construct(CommandBlocker $plugin)
    {
    ...
    public function onPlayerCommand(PlayerCommandPreprocessEvent $event)
    {
        ...
        if ($this->getOwner->isBlocked($cmd)) {
            if ($this->getOwner->isWhiteListed($event->getPlayer())) {
                ...
                $this->log()->info("§aAllowedUser§8» $playername($playerIP) §6tried to execute " . "§c/$cmd " . implode(" ", $words));
            } else {
                $event->setCancelled(true);
                $this->getOwner->logFile($event->getPlayer(), 'Alert', "/$cmd " . implode(" ", $words));
                $this->Log()->alert("§aBlockedUser§8» $playername($playerIP) §6tried to execute " . "§c/$cmd " . implode(" ", $words));
            }
        }
    }
